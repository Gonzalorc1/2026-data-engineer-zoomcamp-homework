"""
Scheduled DAG to process taxi data (yellow and green) from GitHub to BigQuery.
Equivalent to the 09_gcp_taxi_scheduled.yaml flow in Kestra.

This DAG runs automatically:
- Green taxi: 1st day of each month at 9:00 AM (cron: 0 9 1 * *)
- Yellow taxi: 1st day of each month at 10:00 AM (cron: 0 10 1 * *)

Processes the previous month based on the execution date.
"""
...
def get_execution_date_info(**context):
    """Gets year and month from the DAG execution date"""
...
def branch_on_taxi_type(**context):
    ...
    taxi = context['dag_run'].conf.get('taxi', context.get('params', {}).get('taxi', 'green'))
    if taxi == 'yellow':
...
# DAG for Green Taxi (runs on the 1st day of each month at 9:00 AM)
with DAG(
    '09_gcp_taxi_scheduled_green',
    default_args=default_args,
    description='Process green taxi data from GitHub to BigQuery (scheduled)',
    schedule_interval='0 9 1 * *',  # 1st day of each month at 9:00 AM
    start_date=datetime(2024, 1, 1),
    catchup=False,
    params={'taxi': 'green'},
    tags=['gcp', 'taxi', 'bigquery', 'scheduled', 'green'],
) as dag_green:
    ...
    upload_to_gcs = LocalFilesystemToGCSOperator(
        ...
        src='/tmp/{{ params.taxi }}_tripdata_{{ execution_date.strftime("%Y") }}-{{ execution_date.strftime("%m") }}.csv',
        dst='{{ params.taxi }}_tripdata_{{ execution_date.strftime("%Y") }}-{{ execution_date.strftime("%m") }}.csv',
    ...
    with TaskGroup('green_pipeline') as green_pipeline:
        ...
        bq_green_table_ext = BigQueryExecuteQueryOperator(
            ...
            sql=f"""
            CREATE OR REPLACE EXTERNAL TABLE `{GCP_PROJECT_ID}.{GCP_DATASET}.green_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}_ext`
            (
                VendorID STRING,
            ...
            OPTIONS (
                format = 'CSV',
                uris = ['gs://{GCP_BUCKET_NAME}/green_tripdata_{{{{ execution_date.strftime('%Y') }}}}-{{{{ execution_date.strftime('%m') }}}}.csv'],
                skip_leading_rows = 1,
        ...
        bq_green_table_tmp = BigQueryExecuteQueryOperator(
            ...
            sql=f"""
            CREATE OR REPLACE TABLE `{GCP_PROJECT_ID}.{GCP_DATASET}.green_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}`
            AS
            SELECT
              MD5(CONCAT(
              ...
              "green_tripdata_{{{{ execution_date.strftime('%Y') }}}}-{{{{ execution_date.strftime('%m') }}}}.csv" AS filename,
              *
            FROM `{GCP_PROJECT_ID}.{GCP_DATASET}.green_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}_ext`;
            """,
            use_legacy_sql=False,
            location=GCP_LOCATION,
        )
        ...
        bq_green_merge = BigQueryExecuteQueryOperator(
            ...
            USING `{GCP_PROJECT_ID}.{GCP_DATASET}.green_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}` S
    ...
    # Define the DAG flow
    extract >> upload_to_gcs >> green_pipeline >> cleanup

# DAG for Yellow Taxi (runs on the 1st day of each month at 10:00 AM)
with DAG(
    '09_gcp_taxi_scheduled_yellow',
    default_args=default_args,
    description='Process yellow taxi data from GitHub to BigQuery (scheduled)',
    schedule_interval='0 10 1 * *',  # 1st day of each month at 10:00 AM
    start_date=datetime(2024, 1, 1),
    catchup=False,
    params={'taxi': 'yellow'},
    tags=['gcp', 'taxi', 'bigquery', 'scheduled', 'yellow'],
) as dag_yellow:
    ...
    with TaskGroup('yellow_pipeline') as yellow_pipeline:
        ...
        bq_yellow_table_ext = BigQueryExecuteQueryOperator(
            ...
            sql=f"""
            CREATE OR REPLACE EXTERNAL TABLE `{GCP_PROJECT_ID}.{GCP_DATASET}.yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}_ext`
            (
                VendorID STRING,
            ...
            OPTIONS (
                format = 'CSV',
                uris = ['gs://{GCP_BUCKET_NAME}/yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}-{{{{ execution_date.strftime('%m') }}}}.csv'],
                skip_leading_rows = 1,
                ignore_unknown_values = TRUE
            );
            """,
        ...
        bq_yellow_table_tmp = BigQueryExecuteQueryOperator(
            ...
            sql=f"""
            CREATE OR REPLACE TABLE `{GCP_PROJECT_ID}.{GCP_DATASET}.yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}`
            AS
            SELECT
              MD5(CONCAT(
                COALESCE(CAST(VendorID AS STRING), ""),
              ...
              "yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}-{{{{ execution_date.strftime('%m') }}}}.csv" AS filename,
              *
            FROM `{GCP_PROJECT_ID}.{GCP_DATASET}.yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}_ext`;
            """,
            use_legacy_sql=False,
            location=GCP_LOCATION,
        )
        ...
        bq_yellow_merge = BigQueryExecuteQueryOperator(
            ...
            USING `{GCP_PROJECT_ID}.{GCP_DATASET}.yellow_tripdata_{{{{ execution_date.strftime('%Y') }}}}_{{{{ execution_date.strftime('%m') }}}}` S
            ON T.unique_row_id = S.unique_row_id
            WHEN NOT MATCHED THEN
    ...
    # Define the DAG flow
    extract >> upload_to_gcs >> yellow_pipeline >> cleanup
